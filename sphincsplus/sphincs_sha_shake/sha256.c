...
{
  state->i = 0;
  memcpy(state->inbuf, pub_seed, SPX_N);
  memcpy(state->inbuf + SPX_N, addr, SPX_SHA256_ADDR_BYTES);

  state->used_tmp_bytes = 0;
}

/* assumes SPX_N = {16, 24, 32} */
void mgf1_sha256_update(unsigned char *out, mgf1_sha256_state_t* state)
{
  if(SPX_N == SPX_SHA256_OUTPUT_BYTES)
  {
    u32_to_bytes(state->inbuf + SPX_N + SPX_SHA256_ADDR_BYTES, state->i);
    sha256(out, state->inbuf, SPX_N + SPX_SHA256_ADDR_BYTES + 4);
    state->i++;
  }
  else
  {
    if(state->used_tmp_bytes == SPX_N)
    {
      memcpy(out, state->tmp, SPX_N);
      state->used_tmp_bytes = 0;
    }
    else
    {
      unsigned char tmp[state->used_tmp_bytes + SPX_SHA256_OUTPUT_BYTES];
      memcpy(tmp, state->tmp, state->used_tmp_bytes);

      u32_to_bytes(state->inbuf + SPX_N + SPX_SHA256_ADDR_BYTES, state->i);
      sha256(tmp + state->used_tmp_bytes, state->inbuf, SPX_N + SPX_SHA256_ADDR_BYTES + 4);
      state->i++;

      memcpy(out, tmp, SPX_N);
      state->used_tmp_bytes = sizeof(tmp) - SPX_N;
      memcpy(state->tmp, tmp + SPX_N, state->used_tmp_bytes);
    }
  }
}
...
{
  sha256_inc_init(sha256_state->state);
  sha256_state->used_tmp_bytes = 0;
}
...
void sha256_inc_state_update(sha256_inc_state_t *sha256_state, const unsigned char *data, size_t data_len)
{
  ...
  /*
   * first try to fill and empty buffer
   */
  if(sha256_state->used_tmp_bytes)
  {
    // try to fill it
    if(sha256_state->used_tmp_bytes + data_len >= 64)
    {
      processed = 64 - sha256_state->used_tmp_bytes;
      memcpy(sha256_state->tmp + sha256_state->used_tmp_bytes, data, processed);
      sha256_inc_blocks(sha256_state->state, sha256_state->tmp, 1);
      sha256_state->used_tmp_bytes = 0;
    }
    else
    {
      // not enough data to fill it
      memcpy(sha256_state->tmp + sha256_state->used_tmp_bytes, data, data_len);
      sha256_state->used_tmp_bytes += data_len;
      return;
    }
  }

  // now update all remaining data
  unsigned num_blocks = (data_len-processed)/64;
  if(num_blocks)
  {
    sha256_inc_blocks(sha256_state->state, data+processed, num_blocks);
    processed += 64*num_blocks;
  }

  // put remaining data in tmp buffer
  sha256_state->used_tmp_bytes = data_len - processed;
  memcpy(sha256_state->tmp, data+processed, sha256_state->used_tmp_bytes);
}
...
void sha256_inc_state_finalize(sha256_inc_state_t *sha256_state, unsigned char *out)
{
  if(SPX_N < SPX_SHA256_OUTPUT_BYTES)
  {
    unsigned char tmp[SPX_SHA256_OUTPUT_BYTES];
    sha256_inc_finalize(tmp, sha256_state->state, sha256_state->tmp, sha256_state->used_tmp_bytes);
    memcpy(out, tmp, SPX_N);
  }
  else
  {
    sha256_inc_finalize(out, sha256_state->state, sha256_state->tmp, sha256_state->used_tmp_bytes);
  }
}
